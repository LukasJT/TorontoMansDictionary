...
TRIVIA_QUIZ_PATH = ROOT / "trivia-quiz.html"  # hand-written, only read for sitemap check
GUIDE_DIR = ROOT / "guide"  # generated by scripts/build_guide.py, only read for sitemap check
...
def build_sitemap(terms, categories):
    ...
    if TRIVIA_QUIZ_PATH.exists():
        urls.append((f"{SITE_URL}/trivia-quiz.html", "monthly", "0.7"))
    if GUIDE_DIR.exists():
        for p in sorted(GUIDE_DIR.glob("*.html")):
            urls.append((f"{SITE_URL}/guide/{p.name}", "monthly", "0.65"))
    for c in categories:
